...
for i in range(configSectorData[CONFIG_BLOCK_NUM_BUFFERS_RECORDED_POS]):
    ...
    if (dataHeader[BUFFER_HEADER_FRAME_BUFFER_COUNT_POS]%bufferPerFrame == 0):
        ...
        if saveVideo is True:
            im = np.reshape(frame, (configSectorData[CONFIG_BLOCK_WIDTH_POS], configSectorData[CONFIG_BLOCK_HEIGHT_POS] ))
            ...
            thickness              = 1
            lineType               = 2
            val = dataHeader[BUFFER_HEADER_EWL_POS]
            cv2.putText(im,str(val), 
